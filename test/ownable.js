...
contract("Ownable", async accounts => {
    ...
    it("onlyOwner modifier", async () => {
        ...
        try {
            ...
            let res = error.stack.match('Ownable: only owner can call this function');
            res = res ? res[0] : null;
            assert.equal(res, "Ownable: only owner can call this function", "Failed to stop non-owner from calling onlyOwner protected function");
        }
