...
contract("Token", async accounts => {
  it("Staking 100x2", async () => {
    ...
    // Add some tokens on account 1 as weel
    await devToken.mint(1000);
  ...
  it("cannot stake more than owning", async () => {
    ...
    try {
      ...
      let res = error.stack.match('Not enough tokens to stake');
      res = res ? res[0] : null;
      assert.equal(res, "Not enough tokens to stake");
    }
  });
  ...
  it("cant withdraw bigger amount than current stake", async () => {
    ...
    try {
      ...
      let res = error.stack.match('Staking: Cannot withdraw more than you have staked');
      res = res ? res[0] : null;
      assert.equal(res, "Staking: Cannot withdraw more than you have staked", "Failed to notice a too big withdrawal from stake");
    }
  });
  ...
  it("withdraw 50 from a stake", async () => {
    ...
    console.log('200 - withdraw_amount: ' + (200 - withdraw_amount))
  ...
  it("reward stakes", async () => {
    ...
    await devToken.mint(1000);
    // send 1000 tokens to staker
    await devToken.transfer(staker, 1000, { from: accounts[0] });
