...
contract TokenVault is MultiOwnable {
    ...
    function monthlyGiveaway(uint256[] memory amounts, address[] memory addresses) public onlyOwner {
        require(amounts.length == addresses.length, "Amounts and addresses must be the same length.");
        // check if the total amount is less than the contract balance
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < amounts.length; i++) {
            totalAmount += amounts[i];
        }
        require(totalAmount <= IBEP20(bep20TokenAddress).balanceOf(address(this)), "Not enough BEP20 balance in the contract to fulfill the giveaway.");
